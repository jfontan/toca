 ...
 * @discuss		http://groups.google.com/group/jquery-en/browse_thread/thread/480bb6f722b66345
 * @latest		http://www.nicolas.rudas.info/jQuery/Finder/
 * 
 *****************************************************************************
 * Syntax:
...
// Internal debugging. Delete this functions when going to production
	if(!window.debug) {
	...
	log = function() {
		...
		if(window.console) {
			...
			$('body').append('<p>'+d3+': '+$.makeArray(a).join('')+'</p>');}
...
// Default options
	var defaults = {
		...
		onRootReady  : function(rootList,finderObj){},	/*	{Function}		Callback after initial list retrieved via ajax	*/
		onItemSelect : function(listItem,eventTarget,finderObject){},	/*	{Function}		Callback after user clicked on item's list item	*/
		onItemOpen : function(listItem,newColumn,self){},		/*	{Function}		Callback after user clicked on item's anchor	*/
		onFolderSelect : function(listItem,eventTarget,finderObject){},	/*	{Function}		Callback after user clicked on folder's list item	*/
		onFolderOpen : function(listItem,newColumn,self){},	/*	{Function}		Callback after user clicked on fodler's anchor	*/
		processData : function(responseText) {
			return $('<div class="ui-finder-content"/>').append(responseText);
		},
		animate : true,	/*	{Boolean|Number}	Slide in new columns. Number specifies duration	*/
		...
		maxWidth : 450,		/*	{Number}	Maximum width to be allowed for each column		*/
		classNames : {
			'ui-finder' : 'ui-widget ui-widget-header ui-corner-all',
			'ui-finder-wrapper' : 'ui-widget-content ui-corner-all',
			'ui-finder-header' : '',
			'ui-finder-title' : '',
...
// Set some variables (know what we are dealing with)
	...
	// Hover effect for toolbar buttons	
		$('.ui-state-default',this.widget).hover(
	...
	Finder.prototype.select = function(target,noCache,actionType) {
		...
		if(typeof target.length != 'number') {
			debug('Target must be either a URL or a jQuery/DOM element'); return this; }
			
		if(!targetElement[0]) {
			debug('Target element does not exist',target); return this; }
	...
	// Currently selected item will no longer be active
		$('.ui-finder-list-item.ui-state-hover',wrapper).removeClass('ui-state-hover');
	...
	// Remove visible lists which should not be visible anymore
		wrapperLists.each(function(){
			...
			if( finderListLevel >= targetLevel ) {				
				$('.ui-finder-list-item.ui-state-default',finderListWrapper).removeClass('ui-state-default'); 	}
	...
	// Process data before appending new column		
		...
		// If just loaded root, call on root ready callback	
			if(url == 'root' && typeof settings.onRootReady == 'function') {
				settings.onRootReady.apply(self,[data,self]); /*,self.cache[url].response*/ }
...
//			JSONP Version. Miguel Vazquez Garcia Feb 25 2009
	...
	// but make sure that column is appended & parsed (timeout = 0)		
		setTimeout(function(){
			self.adjustWidth(false,newColumn);},0);
	 ...
	 ***/	
	Finder.prototype.adjustWidth = function(ignoreNew,newColumn){
		var self = this,
			wrapper = this.wrapper;
	...
	Finder.prototype.finalise = function(newColumn){
